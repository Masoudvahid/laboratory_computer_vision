...
class ImageMeshEditor:
    ...
    def draw_extended_line(self, x1, y1, x2, y2):
        ...
        for factor in [-1, 1]:
            parallel_line_start = (x1 + factor * perpendicular_dx, y1 + factor * perpendicular_dy)
            parallel_line_end = (x2 + factor * perpendicular_dx, y2 + factor * perpendicular_dy)

            # Check if the parallel lines are valid before drawing
            if not any(np.isnan(parallel_line_start)) and not any(np.isnan(parallel_line_end)):
                self.canvas.create_line(parallel_line_start, parallel_line_end, fill="red", tags="mesh")
